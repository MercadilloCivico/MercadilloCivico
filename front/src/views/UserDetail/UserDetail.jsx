  ...
  return (
    <div className='bg-[white] max-w-2xl mx-auto p-8 rounded-md shadow-lg'>
      ...
      <div className='text-center mt-[4em] text-tuscany-950 py-1.2 px-4'>
        <h2 className='text-center py-2 px-4 mb-0'>{`${user.first_name} ${user.last_name}`}</h2>
        <span>{user.rol}</span>
      </div>
      ...
      <div className='text-justify p-2.5 order-100'>
        <ul className='flex flex-col text-start text-tuscany-950 justify-center'>
          <li>
            <span>
              {`Nombre completo: ${user.first_name} ${user.second_name} ${user.last_name}`}
            </span>
          </li>
          <li>
          ...
          <li>
            <span>{`Suscrito al blog: ${user.subscribe_blog ? 'Sí' : 'No'}`}</span>
          </li>
        </ul>
      </div>
    </div>
